<x-layouts.app :title="$isTeacherReport ? 'Mis evaluaciones' : 'Reportes'">
    @php
        $exportQuery = $isTeacherReport
            ? array_merge(request()->query(), ['report' => 'evaluations'])
            : request()->query();
        $tableTitle = $isTeacherReport ? 'Calificaciones recibidas' : $title;
    @endphp

    <div class="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
            <h2 class="text-2xl font-bold text-[#123b7a]">{{ $isTeacherReport ? 'Mis evaluaciones' : 'Reportes' }}</h2>
            <p class="text-slate-600">{{ $isTeacherReport ? 'Consulta las calificaciones recibidas por alumnos y apoderados.' : 'Tablas filtrables para gestión y exportación.' }}</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a class="school-button-secondary rounded-md px-4 py-2 text-sm font-semibold" href="{{ route('reports.export', ['format' => 'excel'] + $exportQuery) }}">Excel</a>
            <a class="school-button-primary rounded-md px-4 py-2 text-sm font-semibold" href="{{ route('reports.export', ['format' => 'pdf'] + $exportQuery) }}">PDF</a>
        </div>
    </div>

    <form class="school-panel rounded-lg p-5" method="GET" action="{{ route('reports.index') }}">
        @if($isTeacherReport)
            <input type="hidden" name="report" value="evaluations">
        @endif

        <div class="grid gap-4 md:grid-cols-4">
            @unless($isTeacherReport)
                <div>
            ...
            <div>
                ...
                <input class="mt-1 w-full rounded-md px-3 py-2" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="{{ $isTeacherReport ? 'Alumno, apoderado o comentario' : 'Nombre, código o DNI' }}">
            </div>
            @unless($isTeacherReport)
                <div>
    ...
    <section class="school-panel mt-6 overflow-hidden rounded-lg">
        <div class="flex flex-col justify-between gap-2 border-b border-slate-200 px-4 py-3 md:flex-row md:items-center">
            <div>
                <h3 class="font-semibold text-[#123b7a]">{{ $tableTitle }}</h3>
